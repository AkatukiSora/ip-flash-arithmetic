...
test.describe('ホームページコンポーネント', () => {
  ...
  test('ナビゲーションリンクが正常に動作する', async ({ page }) => {
    ...
    await expect(page).toHaveURL(/\/calculator\/?/);    // ホームに戻る
    await page.goto('/');
...
test.describe('クイズコンポーネント', () => {
  ...
  test('問題タイプ選択が正常に動作する', async ({ page }) => {
    ...
    // 各問題タイプを選択
    await selector.selectOption('BINARY_IP_CONVERSION');
    await expect(page.locator('text=2進数表記')).toBeVisible();
    
    await selector.selectOption('CIDR_TO_SUBNET');
    // option要素は通常非表示なので、代わりにselectの値で確認
    await expect(selector).toHaveValue('CIDR_TO_SUBNET');
    
    await selector.selectOption('NETWORK_ADDRESS');
    await expect(selector).toHaveValue('NETWORK_ADDRESS');
  ...
  test('スコア表示が正常に動作する', async ({ page }) => {
    ...
    // スコアが更新される
    await expect(page.locator('text=スコア: 1/')).toBeVisible();
  });
...
test.describe('練習コンポーネント', () => {
  ...
  test('問題タイプ切り替えが即座に反映される', async ({ page }) => {
    ...
    await expect(page.getByRole('heading').filter({ hasText: 'ブロードキャストアドレス' })).toBeVisible();
    
    // 戻ってから別の問題タイプに変更
    await page.getByRole('link', { name: '← 戻る' }).click();
    await page.click('text=ネットワークアドレス計算');
    await expect(page.getByRole('heading').filter({ hasText: 'ネットワークアドレス' })).toBeVisible();
  });
});
